...
DEBUG ?= true
DEBUG_FLAGS ?= dnsserver flashcfg html httpclient httpserver json ota rtc system tcpserver wifi \
               gpio hspi uart                                                                   \
               api apiserver config core device
...
FW_BASE_URL      ?= http://provisioning.example.com
FW_BASE_OTA_PATH ?= /firmware/esphlg
...
CFLAGS += -Wpointer-arith -Wall -Wl,-EL -fno-inline-functions -nostdlib -mlongcalls -mtext-section-literals \
          ...
          -D__ets__ -DICACHE_FLASH -DUSE_OPTIMIZE_PRINTF -DFLASH_CONFIG_ADDR=$(FLASH_CONFIG_ADDR) -D_OTA \
          -Os -O2 -std=c99
