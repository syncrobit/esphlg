...
static void  ICACHE_FLASH_ATTR  listen(void);
static void  ICACHE_FLASH_ATTR  reset(void);
static void  ICACHE_FLASH_ATTR  on_client_recv(void *arg, char *data, uint16 len);
static void  ICACHE_FLASH_ATTR  process_request(uint8 *request, uint16 len, uint8 remote_ip[], uint16 remote_port);
static uint8 ICACHE_FLASH_ATTR *prepare_response(uint8 *request, uint16 *len);
static uint8 ICACHE_FLASH_ATTR *prepare_ip_response(
...
static uint8 ICACHE_FLASH_ATTR *prepare_error_response(
                                    dns_header_t *request_header,
                                    uint8 reply_code,
                                    uint8 *query,
                                    uint16 query_len,
