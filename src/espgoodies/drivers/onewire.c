...
static void ICACHE_FLASH_ATTR write_bit(one_wire_t *one_wire, bool value);
static bool ICACHE_FLASH_ATTR read_bit(one_wire_t *one_wire);
...
void one_wire_search_reset(one_wire_t *one_wire) {
    ...
    memset(one_wire->rom, 0, sizeof(uint8) * 8);
}
...
bool one_wire_search(one_wire_t *one_wire, uint8 *addr) {
    ...
    if (!search_result || !one_wire->rom[0]) {
        ...
        memcpy(addr, one_wire->rom, sizeof(uint8) * 8);
    }
